# Work Packages: 장소 추가 및 장소 데이터 검증
...
- Duplicate detection: Two-phase (SQL bounding box ±0.001° → app-level 100m + 80% similarity)
...
### Implementation Notes
- Validation order: DTO → RateLimitGuard → ProfanityFilterPipe → Service (duplicate detection)
- Create sequence: Place (if new) → UserPlace → PlaceModerationQueue (status: pending)
...
**Independent Test**: Complete user journey works (add place → see pending status → admin approves → place visible); all error cases handled gracefully; quickstart.md validated.
...
- WP01-WP02: Sequential (schema → infrastructure)
