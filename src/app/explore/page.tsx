...
export default function ExplorePage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8">여행 탐색</h1>
      ...
      {loading ? (
        <div className="flex justify-center py-10">
          <div className="animate-pulse text-gray-500">여행 정보를 불러오는 중...</div>
        </div>
      ) : publicTrips.length === 0 ? (
        <div className="bg-gray-50 p-8 rounded-lg text-center">
          <p className="text-gray-600 mb-4">아직 공개된 여행 계획이 없습니다.</p>
          <Link 
            href="/trips/new" 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {publicTrips.map((trip) => (
            ...
            >
              ...
              <div className="p-4">
                {trip.description && (
                  <p className="text-gray-700 line-clamp-2 mb-3">{trip.description}</p>
                )}
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-blue-600">{trip.profiles?.username || '사용자'}</span>
                </div>
              </div>
            </Link>
          ))}
